...
import system.models.entity.NhanVien; // Import NhanVien model
...
public class NhanVienView extends JPanel {
    ...
    public void displayNhanVienDetails(NhanVien nhanVien) {
        if (nhanVien != null) {
            ...
            txtLuong.setText(String.valueOf(nhanVien.getLuong())); // Convert int to String
    ...
    public NhanVien getNhanVienFromInput() {
        ...
        if (nv.getHoTen().trim().isEmpty() || nv.getCccd().trim().isEmpty() || nv.getSdt().trim().isEmpty() || nv.getLuong() <= 0) {
