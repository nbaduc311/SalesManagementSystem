...
public class PhieuNhapView extends JPanel {
     ...
     */
    public void updateProductPriceInImportCart(int rowIndex, BigDecimal newUnitPrice) {
        if (rowIndex >= 0 && rowIndex < currentImportCartItems.size()) {
            ChiTietPhieuNhap ctpn = currentImportCartItems.get(rowIndex);
    ...
    public void updateTotalImportAmountLabel() {
        ...
        for (ChiTietPhieuNhap item : currentImportCartItems) {
            currentTotalImportAmount += item.getThanhTien().doubleValue();
        }
    ...
    // NEW: Getter for lblSelectedProductUnitPrice text
    public String getLblSelectedProductUnitPriceText() {
        return lblSelectedProductUnitPrice.getText();
    }
